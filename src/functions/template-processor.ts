...
async function replaceProcessor(_cur: RegExpExecArray, _org: string): Promise<string> {
    return _cur[1];
}

async function diffPatchProcessor(_cur: RegExpExecArray, _org: string): Promise<string> {
    try {
...
async function ejsProcessor(_cur: RegExpExecArray, _org: string): Promise<string> {
    ...
    const ctx = ejs.prepareContext({ current: _cur, original: _org });
    try {
...
async function varYamlProcessor(_cur: RegExpExecArray, _org: string): Promise<string> {
    let doc = null;
    try {
    ...
    const result = _.merge(variables, doc);
...
async function varJsonProcessor(_cur: RegExpExecArray, _org: string): Promise<string> {
    let doc = null;
    try {
...
async function varJsonPatchProcessor(_cur: RegExpExecArray, _org: string): Promise<string> {
    let doc = null;
    try {
        doc = JSON.parse(jsonrepair(_cur[1]));
    } catch(e) {
    ...
    const result = jsonPatch(variables, doc);
        ...
        switch (op) {
            ...
            case 'remove': {
                if (!_.unset(newDoc, lodashPath)) {
                    ...
                    toastr.warning(`Path "${path}" could not be removed.`, 'JSON Patch');
                }
                break;
            }
